...
export default function SubmitWord({ session, playerId }) {
  ...
  function handleSubmit(e) {
    ...
    if (!/^[a-z]+$/.test(cleaned)) { setError('Letters only — no spaces or numbers.'); return; }
  ...
  const diffClass = difficulty === 'easy' ? 'badge-teal' : difficulty === 'hard' ? 'badge-gold' : 'badge-ink';

  return (
    <div className="page-center">
      <div className="container-sm">
        <div className="text-center" style={{ marginBottom: 32 }}>
          <span className="badge badge-teal" style={{ marginBottom: 8, display: 'inline-block' }}>Round {session.currentRound || 1} of {session.rounds}</span>
          <h1 style={{ fontSize: 38, marginBottom: 8 }}>Submit Your Word</h1>
          <div className="flex gap-8 justify-center" style={{ marginTop: 8 }}>
            <span className={`badge ${isFunMode ? 'badge-gold' : 'badge-teal'}`}>{isFunMode ? '🎉 Fun Mode' : '📚 Education Mode'}</span>
            <span className={`badge ${diffClass}`}>{diffBadge}</span>
          </div>
        </div>
        ...
        <div className="panel" style={{ marginBottom: 24 }}>
          ...
          <div style={{ height: 8, background: 'var(--blush)', borderRadius: 4, overflow: 'hidden', marginBottom: 16 }}>
            <div style={{ height: '100%', borderRadius: 4, background: 'var(--teal)', width: `${(submittedCount / totalPlayers) * 100}%`, transition: 'width 0.4s ease' }} />
          </div>
          {session.players.map(p => (
            <div key={p.id} className="flex items-center gap-8" style={{ marginBottom: 8 }}>
              <div style={{ width: 10, height: 10, borderRadius: '50%', background: session.wordSubmissions?.[p.id] ? 'var(--success)' : 'var(--border)' }} />
              <span style={{ fontSize: 14, color: p.id === playerId ? 'var(--ink)' : 'var(--muted)', fontWeight: p.id === playerId ? 600 : 400 }}>
                {p.name} {p.id === playerId ? '(you)' : ''}
              </span>
              {session.wordSubmissions?.[p.id] && <span style={{ marginLeft: 'auto', fontSize: 13, color: 'var(--success)', fontWeight: 600 }}>Ready ✓</span>}
            </div>
          ))}
        </div>
        ...
        {!mySubmission ? (
          <div className="panel">
            {error && <div style={{ background: '#FEE', border: '1px solid var(--danger)', borderRadius: 'var(--radius-md)', padding: '10px 14px', marginBottom: 16, fontSize: 14, color: 'var(--danger)' }}>{error}</div>}
            <form onSubmit={handleSubmit} className="flex-col gap-16">
              <div>
                ...
                <input className="input" value={word} onChange={e => setWord(e.target.value)}
                  ...
                  autoFocus disabled={submitted} style={{ fontSize: 20, textAlign: 'center' }} />
              </div>
          ...
          <div className="panel text-center">
            ...
            <p style={{ color: 'var(--muted)', fontSize: 14 }}>Waiting for all players…</p>
          </div>
        )}
        ...
        {session.phase === 'loading' && (
          <div className="panel text-center" style={{ marginTop: 16 }}>
            ...
            <p style={{ color: 'var(--muted)' }}>Generating words and dealing cards…</p>
          </div>
        )}
      </div>
    </div>
